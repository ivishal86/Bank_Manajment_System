...
def search():
    try:
        ...
        cursor = conn.cursor()
        cursor.execute("select EmpID, EmpName, EmpMail, EmpPass, EmpNumber, Age, EmpGender, HireDate from employee where EmpID = '" +str(EmpID.get())+ "'")
...
def delete():
    try:
     conn = mysql.connector.connect(host='localhost',database='bank_mang',user='root',password='root',charset='utf8')
     cursor = conn.cursor()
     row = cursor.execute("delete from employee where EmpID='"+str(EmpID.get())+"' ")
     if(cursor.rowcount>0):
         ...
         mBox.showerror('Error','Cannot delete. Check the details.')
    except Error as e :
     print("Error while connecting to MySQL", e)
    finally:
        print("MySQL connection is closed")
...
#Search
search=tk.Button(win8,font=('times new roman',16), text="Search For Acc",command=search)
search.grid(column=4, row=2,columnspan=2)

#Delete
Delete=tk.Button(win8,font=('times new roman',16), text="Delete The Acc",command=delete)
Delete.grid(column=4, row=3,columnspan=2)

#Back to Menu
back=tk.Button(win8,font=('times new roman',16), text="Emp. Menu",command=Emp)
